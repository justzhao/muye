	...
	if(cowID!=null)
	{
		...
		{ 
			...
			jo.put("Operator", c.getOperator());
					...
					ChartData=c.getTotalMilks()+ChartData;
					if (!ChartX.equals("")) {
						ChartX = ","+ChartX;
					}
					ChartX = "'" + c.getTestDate().substring(5) + "'"+ChartX;
					
					  
					if (MaxY.equals(""))
					{
						MaxY=c.getTotalMilks();
					}else
					{
						//Double.parseDouble(str);
					
						if(Double.parseDouble(MaxY)<Double.parseDouble(c.getTotalMilks()))
						{
							MaxY=String.valueOf(Double.parseDouble(c.getTotalMilks()));
						}
					}
					if(MinY.equals(""))
					{
				      MinY=c.getTotalMilks();
					}else
					{
						if(Double.parseDouble(MinY)>Double.parseDouble(c.getTotalMilks()))
						{
							MinY=String.valueOf(Double.parseDouble(c.getTotalMilks()));
						}
					}
		...
		// logger.debug("the chartx is "+ ChartX);
